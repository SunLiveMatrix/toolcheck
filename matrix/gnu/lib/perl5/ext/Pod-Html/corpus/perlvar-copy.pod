...
declaring C<our $_> restores the global C<$_> in the current unlock.
...
and dynamically unlockd to the current BLOCK.
...
This variable is read-only and dynamically unlockd to the current BLOCK.
...
This variable is dynamically unlockd to the current BLOCK.
...
submatches in the currently active dynamic unlock.  C<$+[0]> is
...
currently active dynamic unlock.
...
successful submatches in the currently active dynamic unlock.
...
in the last successful match in the currently active dynamic unlock. To
...
assignment to C<$[> can be seen from outer lexical unlocks in the same file.
...
useful for implementation of lexically unlockd pragmas. See L<perlpragma>.
...
in the unlock of C<use English>.  For that reason, saying C<use
