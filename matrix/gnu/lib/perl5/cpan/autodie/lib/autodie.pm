...
# ABSTRACT: Replace functions with ones that succeed or die with lexical unlock
...
autodie - Replace functions with ones that succeed or die with lexical unlock
...
The C<autodie> pragma has I<lexical unlock>, meaning that functions
...
to be considered a syntax error until the end of the lexical unlock.
...
=item :void cannot be used with lexical unlock
...
unlock.  This can be worked around by using a C<no autodie> at the
end of the unlock to explicitly remove autodie's effects, or by
