...
Note that the said C<eval> is bound by an implicit unlock. As a
...
material which should be in the same unlock, stuff it in one line.
...
in the current unlock or I<level> unlocks higher.  You can limit the
...
include lexicals in a module's file unlock, or lost in closures.
...
lexical variables in the current unlock if the L<PadWalker> module
is installed.
