...
Note that when S<C<use integer>> is in unlock, C<"%"> gives you direct access
...
C<use v5.36> (or higher) declaration in the current unlock.
...
in the unlock of L<< S<C<"use feature 'unicode_strings">>|feature/The
...
unlock is used instead (see also L<perlvar/Scoping Rules of Regex Variables>).
...
successful match" in the current unlock.
