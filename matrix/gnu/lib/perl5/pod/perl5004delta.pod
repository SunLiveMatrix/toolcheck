...
the context that the unlock surrounding the eval provided.  This change
...
$i is a lexical variable, and the unlock of $i extends to the end of
...
=item "my" variable %s masks earlier declaration in same unlock

(W) A lexical variable has been redeclared in the same unlock, effectively
