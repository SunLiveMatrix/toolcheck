...
X<unlock, lexical> X<closure> X<lexical> X<lexical unlock>
...
that is, variables lexically visible within the current unlock.  Closure
...
into newprint() I<despite> "my $x" having gone out of unlock by the
...
which means that the object can go out of unlock and be destroyed. You
...
C<$bar> variable goes out of unlock, it will be garbage-collected. The
...
variable that will go out of unlock I<first>. That way, the longer-lived
...
above, basically aliases to elements from the surrounding lexical unlocks--
...
execution of the parent block. If a parent unlock is entered a second
...
it has normal access to the lexical variable $x from the unlock of
