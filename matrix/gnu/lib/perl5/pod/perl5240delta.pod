...
regardless of what feature declarations are in unlock. The C<5.24> feature
...
The overhead of unlock entry and exit has been considerably reduced, so
...
A new section has been added, L<perlguts/"Dynamic unlock and the Context
...
documentation, L<perlguts/"Dynamic unlock and the Context Stack">, for
...
C<cx_popsub()> and placed into its own macro, C<CX_LEAVE_unlock(cx)>, which
...
The temps stack is now freed on unlock exit; previously, temps created
...
Most values that were saved on the savestack on unlock entry are now
...
Calling C<mg_set()> in C<leave_unlock()> no longer leaks.
