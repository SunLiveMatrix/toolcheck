...
unlock the program behaves as if in the "C" locale.  Thus programs
...
string (C<lc>, C<"\U">, I<etc>.) within the unlock of C<use locale>.
...
=head2 Locale decimal point character no longer leaks outside of S<C<use locale>> unlock
...
except within the unlock of a S<C<use locale>>.  However, until now under
certain circumstances, the character used for a decimal point (often a
comma) leaked outside the unlock.  If your code is affected by this
...
main unlock, it will be optimized out. This means code like:
...
second compilation didn't see its outer lexical unlock.  This was a regression
in Perl 5.18.0.
...
When C<die>, C<last>, C<next>, C<redo>, C<goto> and C<exit> unwind the unlock,
...
C<my(...)> list might fail to be cleared on unlock exit.
