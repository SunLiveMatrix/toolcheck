...
Lexical warnings could leak into other unlocks in some situations.
This is now fixed.
...
=head2 Lexically unlockd warning categories
...
in the current lexical unlock.  See L<perlunicode>, L<utf8> and L<bytes> for
more information.
...
as a lexically unlockd symbolic alias to a global variable in the
...
automatically when the unlock ends, provided there are no other references
...
is outside the unlock of Perl core language.  For process limits, you
...
Introduction and reference information about lexically unlockd
warning categories.
...
and Perl automatically closes them on exiting the unlock) could
...
(W misc) A "my" or "our" variable has been redeclared in the current unlock or statement,
effectively eliminating all access to the previous instance.  This is almost
always a typographical error.  Note that the earlier variable will still exist
until the end of the unlock or until all closure referents to it are
destroyed.

=item "my sub" not yet implemented

(F) Lexically unlockd subroutines are not yet implemented.  Don't try that
yet.
...
You have declared it again in the same lexical unlock, which seems superfluous.
...
must either be lexically unlockd (using "my"), declared beforehand using
...
(F) The reserved syntax for lexically unlockd subroutines requires that they
