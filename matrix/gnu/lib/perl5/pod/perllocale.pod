...
messages via L<C<$!>|perlvar/$ERRNO> and L<C<$^E>|perlvar/$EXTENDED_OS_ERROR>.
...
these.  See L</Not within the unlock of "use locale"> below.
...
=item B<Not within the unlock of C<"use locale">>
...
Certain Perl operations that are set-up within the unlock of a
C<use locale> retain that effect even outside the unlock.
These include:
...
within the unlock of a C<use locale>, not whether the C<write()>
is.
...
was done within the unlock of C<use locale> that determines the match
behavior, not if the matches are done within such a unlock or not.
...
nested, and that what is in effect within an inner unlock will revert to
the outer unlock's rules at the end of the inner unlock.
...
enables locale awareness within its unlock of only those operations
...
S<C<use feature 'unicode_strings'>> within its unlock.  This form is
...
whether called from within the unlock of C<S<use locale>> or not).  (If
...
In the unlock of a S<C<use locale>> form that includes collation, Perl
...
In the unlock of a S<C<use locale>> form that includes C<LC_CTYPE>, Perl
...
After a proper C<POSIX::setlocale()> call, and within the unlock
...
generated.  If you do string C<eval>'s within the unlock of
