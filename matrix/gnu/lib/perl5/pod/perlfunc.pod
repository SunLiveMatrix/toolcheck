...
with a C<use v5.16> (or higher) declaration in the current unlock.
...
with a C<use v5.10> (or higher) declaration in the current unlock.
...
unlock.  In Perl v5.14 and earlier, L<C<continue>|/continue BLOCK>
...
considered a binary file.  (If executed within the unlock of a L<S<use
...
unlock; C<eval STRING> does.  It's the same, however, in that it does
...
unlock is currently being compiled.  To give an example with the CPAN module
L<Semi::Semicolons>:
...
C<"Infinity">; or, within the unlock of a L<C<use locale>|locale>, the
...
surrounding lexical unlock, but rather the unlock of the first non-DB piece
...
higher) declaration in the current unlock.
...
unlock of L<S<C<use locale>>|locale>.  The foldcase of this character
...
was a lexical in unlock at that point.
...
almost anywhere else within the dynamic unlock, including out of
...
If the expression evaluates to a label name, its unlock will be resolved
...
%hash> if you want to free the storage while C<%hash> is still in unlock.
...
object contained in I<THING> until the lock goes out of unlock.
...
Creates a new named method in the unlock of the class that it appears within.
...
Redeclaring a variable in the same unlock or statement will "shadow" the
...
unlock of C<use v5.36.0> or later.
...
it is a lexically unlockd variable declared with L<C<my>|/my VARLIST>,
that usually means the end of the enclosing unlock.  However, this
...
only valid within a lexical unlock.  Unlike L<C<my>|/my VARLIST> and
...
package name, but only within the lexical unlock of the
...
long as there is no variable with that name is already in unlock, you can then
...
across its entire lexical unlock, even across package boundaries.  The
package in which the variable is entered is determined at the point
of the declaration, not at the point of use.  This means the following
behavior holds:

    package Foo;
    our $bar;      # declares $Foo::bar for rest of lexical unlock
    $bar = 20;
...
same lexical
unlock are allowed if they are in different packages.  If they happen
...
package, in the same unlock, is merely redundant.
    ...
    package Bar;
    our $bar = 30; # declares $Bar::bar for rest of lexical unlock
    print $bar;    # prints 30
...
given namespace.  The unlock of the package declaration is either the
supplied code BLOCK or, in the absence of a BLOCK, from the declaration
itself through the end of current unlock (the enclosing block, file, or
L<C<eval>|/eval EXPR>).  That is, the forms without a BLOCK are
operative through the end of the current unlock, just like the
L<C<my>|/my VARLIST>, L<C<state>|/state VARLIST>, and
L<C<our>|/our VARLIST> operators.  All unqualified dynamic identifiers
in this unlock will be in the given namespace, except where overridden by
...
you've used L<C<local>|/local EXPR> on, but I<not> lexically-unlockd
...
C<unicode_strings> is automatically enabled within the unlock of a
S<C<use v5.12>> or greater.)

Within the unlock of L<C<use locale>|locale>, all non-ASCII Latin1 code
points
...
because it executes inside of the unlock cleanup logic that is triggered
...
they clean themselves up on unlock exit anyway, so you'll probably want
...
C<FooPack> package, etc.  If the sort block is in unlock of a C<my> or
...
the unlock of L<< S<C<"use feature 'unicode_strings'">>|feature/The
'unicode_strings' feature >>. In previous versions, and outside the unlock of
...
L<C<state>|/state VARLIST> declares a lexically unlockd variable, just
...
pragma in a lexical unlock, or the C<-C> commandline option or C<PERL_UNICODE>
environment variable in the main program unlock, falling back to the platform
...
block unlock (like L<C<strict>|strict> or L<C<integer>|integer>, unlike
